...
mutare_fisiere_local(){
    ...
    cautare_fisiere . | xargs -0 -I {} mv {} "$destinatie" && echo "Fișierele au fost mutate în $destinatie." || echo "Eroare la mutarea fișierelor."
}
...
mutare_fisiere_git(){
    ...
    cautare_fisiere . | xargs -0 -I {} mv "{}" "$backup" && echo "Fișierele au fost mutate în $backup." || echo "Eroare la mutarea fișierelor."    
    if git diff-index --quiet HEAD --; then
        echo "Nicio modificare de commitat."
    else
